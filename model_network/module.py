...
class ConvSelfAttention(nn.Module):
    def __init__(self, self_feature_size, heads, dim_head, in_channels = 64, kernel_size = (3, 3), q_stride = 1, k_stride = 1, v_stride = 1):
        ...
        '''
        self.self_feature_size   = self_feature_size
        self.heads        = heads
    ...
    def forward(self, features):
        ...
        features = rearrange(features, 'b (l w) n -> b n l w', l = self.self_feature_size[0], w = self.self_feature_size[1])
...
class ConvCrossAttention(nn.Module):
    def __init__(self, self_feature_size, cross_faetures_size, heads, dim_head, in_channels = 64, kernel_size = (3, 3), q_stride = 1, k_stride = 1, v_stride = 1):
        ...
        Args
            self_feature_size:  셀프 피처맵의 사이즈
            cross_feature_size: 크로스 피처맵의 사이즈
            heads:              헤드의 갯수
            dim_head:           헤드 하나의 차원 수
            in_channels:        어텐션 모듈에 입력받을 피처맵의 채널 ([B, C, H, W]에서 C에 해당)
            kernel_size:        (kernel_height, kernel_width),
        ...
        '''
        self.self_feature_size   = self_feature_size
        self.cross_features_size = cross_faetures_size
        self.heads        = heads
        self.dim_head     = dim_head
        self.in_channels  = in_channels
        self.out_channels = dim_head * heads
    ...
    def forward(self, self_features, cross_features):
        ...
        self_features  = rearrange(
                            ...
                            l = self.self_feature_size[0], 
                            w = self.self_feature_size[1])
        cross_features = rearrange(
                            cross_features, 'b (l w) n -> b n l w', 
                            l = self.cross_features_size[0], 
                            w = self.cross_features_size[1])
        # cross_features = F.interpolate(input = cross_features,
        #                                 size = (self.self_feature_size[0], self.self_feature_size[1]), 
        #                                 mode = "bilinear",
        #                                 align_corners = True)
        self_features  = F.interpolate(input = self_features,
                                        size = (self.cross_features_size[0], self.cross_features_size[1]), 
                                        mode = "bilinear",
                                        align_corners = True)
