...
feature -> Attention -> LayerNorm -> FeedForward -> LayerNorm
features1: Q
features2: K and V
"""
class SelfConvTransfomer(nn.Module):
    ...
    def forward(self, features):
        ...
        for pre_norm1, attention, pre_norm2, feedforward in self.module:
            features = attention(pre_norm1(features), pre_norm1(features)) + features
...
class CrossConvTransformer(nn.Module):
    def __init__(self, size, in_channels, depth = 1, heads = 8, dim_head = 8, dim_mlp = 64):
        ...
        self.cross_norm1       = LayerNorm(in_channels)
        self.cross_norm2       = LayerNorm(in_channels)
        self.cross_attention   = ConvolutionAttention(
                                    size = size, in_channels = in_channels,  heads = heads, dim_head = dim_head)
        self.cross_norm3       = LayerNorm(in_channels) 
    ...
    def forward(self, features1, features2):
        ...
        Args:
            features1: [B, C, H, W] ~ Q
            features2: [B, C, H, W] ~ K, V
        returns
            features: [B, C, H, W]
        """
        # 피처를 (B C H W) -> (B HW C)으로 shape을 바꾸는 코드
        features1 = self.forward_shape(features1)
        features2 = self.forward_shape(features2)
        
        # 셀프 피처와 크로스 피처를 사용해서, 크로스 어텐션 계산하는 코드
        features = self.cross_attention(self.cross_norm1(features1), self.cross_norm2(features2)) + features2
        features = self.cross_feedforward(self.cross_norm3(features)) + features

        for pre_norm1, attention, pre_norm2, feedforward in self.module: # 셀프 어텐션 계산을 돌리는 반복문
            features = attention(pre_norm1(features), pre_norm1(features)) + features
            features = feedforward(pre_norm2(features)) + features
